...
public sealed class SourceFetchTransport(HttpClient httpClient, EvidenceGenerationOptions options) : ISourceFetchTransport
{
	...
	public async Task<SourceFetchResult> FetchAsync(SourceFetchRequest request, CancellationToken cancellationToken)
	{
		...
		timeout.CancelAfter(TimeSpan.FromSeconds(options.FetchTimeoutSeconds));
		CancellationToken fetchToken = timeout.Token;
		try
		{
			...
			for (int redirects = 0; ; redirects++)
			{
				...
				using HttpResponseMessage response = await httpClient.SendAsync(message, HttpCompletionOption.ResponseHeadersRead, fetchToken);
				...
				await using Stream source = await response.Content.ReadAsStreamAsync(fetchToken);
				using MemoryStream bounded = new();
				byte[] buffer = new byte[16 * 1024];
				while (true)
				{
					int read = await source.ReadAsync(buffer, fetchToken);
					if (read == 0) break;
				...
				try
				{
					JsonDocument document = await JsonDocument.ParseAsync(bounded, cancellationToken: fetchToken);
		...
		{
			throw new EvidenceSourceException("fetch_timeout", "The source fetch exceeded the configured timeout.", exception);
		}
	}
