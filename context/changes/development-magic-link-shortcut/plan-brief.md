...
| Redirect handoff | Purpose-built protected-cookie helper | Avoids putting the raw token in the redirect query string or browser history without adding MVC TempData/ViewFeatures. |
...
- Purpose-built protected-cookie one-time handoff from request endpoint to login page.
...
The existing auth flow remains authoritative: token generation, hashing, default send failure cleanup, 15-minute expiry, one-use consumption, and local return URL normalization stay in `MagicLinkService`. The request endpoint opts into receiving the generated link only when `Auth:ShowMagicLinksInFrontend` is true, can show that persisted link even when SMTP is unavailable, stores it briefly through a protected-cookie helper for the redirected login page, and the login page renders it with warning copy.

## Phases at a Glance

| Phase | What it delivers | Key risk |
| --- | --- | --- |
| 1. Shortcut Contract And Configuration | Config flag, optional result link, one-time cookie handoff | Accidentally exposing raw links when the flag is disabled |
...
- A purpose-built protected-cookie handoff is suitable for this single-link display; no multi-instance link archive is needed.
...
- Auth/privacy regression tests continue to pass, and new tests cover default/no-link behavior, stale handoff clearing, and the enabled no-SMTP shortcut path.
