...
|---|--------------------------|--------|------------|--------------------------------|
| 1 | Passwordless auth signs a user in from an invalid, expired, consumed, or replayed magic link, or reveals whether an email exists. | High | High | interview Q1/Q3/Q4; PRD Access Control; archive passwordless-member-access; hot-spot dirs `LinuxGameCompat/Services` and `LinuxGameCompat/Program.cs` |
| 2 | User email, token, or auth material leaks through persistence, logging, email delivery, or error behavior. | High | Medium | interview Q4; tech-stack auth enabled; archive passwordless-member-access |
...
|------|-----------------------------|----------------|--------------------------------------|-----------------------|-----------------------|
| #1 | Invalid, expired, consumed, and replayed links do not sign in; request responses stay generic; unsafe return URLs do not redirect externally. | Happy-path login means auth is safe. | Auth entry points, token lifecycle, persisted request state, sign-in boundary, redirect normalization, email-existence behavior. | integration/service tests first; browser smoke only where service tests cannot observe the failure. | happy-path-only auth tests; over-mocking identity/cookie behavior; asserting implementation internals instead of sign-in outcome. |
| #2 | Stored, logged, sent, and failed-email artifacts do not expose raw tokens or more user-email/account information than required for operation. | Hashing the token is enough to cover privacy. | Token generation/storage boundary, email sender/logging boundary, error paths, database fields that intentionally retain email identifiers. | integration tests with fake sender/logger and persistence checks. | copied production calculation as oracle; testing only success path; ignoring failure logs. |
...
|---|------------|-----------------|---------------|------------|--------|---------------|
| 1 | Auth and privacy regression floor | Defend magic-link auth, redirects, replay, generic responses, and email/token privacy at the cheapest layer. | #1, #2 | integration + service | change opened | testing-auth-privacy-regression-floor |
